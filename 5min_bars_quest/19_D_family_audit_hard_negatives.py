# Family D hard-negative univariate side audit.
#
# Purpose:
# Evaluate each side-native Family D feature one at a time against frozen
# episode-entry labels, using a harder sampled negative set instead of all
# non-entry bars.
#
# Label definition:
...
#
# Source selection:
# This audit prefers df_feat_reduced_C when it exists, so prior Family B and
# Family C pruning is already reflected in the working feature frame.
# If that frame is unavailable, it falls back to df_feat_reduced_B,
# then df_feat_reduced, then df_feat.
#
# Side separation:
# Long audit uses only Family D up-side features.
# Short audit uses only Family D down-side features.
...
#
# Output meaning:
# Each output row is one univariate Family D feature audit under the
...
#
# Saved tables:
# D_long_uni_hard and D_short_uni_hard keep the full long-side and short-side
...
# Otherwise fall back to the Family B pruned frame,
# then the broader reduced frame, then the full frame.
if "df_feat_reduced_C" in globals():
    df_feat_D_src = df_feat_reduced_C.copy()
elif "df_feat_reduced_B" in globals():
    df_feat_D_src = df_feat_reduced_B.copy()
elif "df_feat_reduced" in globals():
    df_feat_D_src = df_feat_reduced.copy()
elif "df_feat" in globals():
    df_feat_D_src = df_feat.copy()
else:
...
# -----------------------------
# exact Family D side-native columns
# -----------------------------
# Long audit uses only up-native Family D columns.
famD_long_cols = sorted([
    c for c in df_audit.columns
    if c.startswith("famD_") and ("_up_" in c or c.endswith("_up"))
])

# Short audit uses only down-native Family D columns.
famD_short_cols = sorted([
    c for c in df_audit.columns
    if c.startswith("famD_") and ("_dn_" in c or c.endswith("_dn"))
])

print("Family D long-native cols :", len(famD_long_cols))
print("Family D short-native cols:", len(famD_short_cols))
...
train_sample_long, train_pos_long, train_neg_long = build_hard_sample_mask("y_long", train_mask_all, rng_seed=RNG_SEED + 71)
test_sample_long,  test_pos_long,  test_neg_long  = build_hard_sample_mask("y_long", test_mask_all,  rng_seed=RNG_SEED + 72)

train_sample_short, train_pos_short, train_neg_short = build_hard_sample_mask("y_short", train_mask_all, rng_seed=RNG_SEED + 81)
test_sample_short,  test_pos_short,  test_neg_short  = build_hard_sample_mask("y_short", test_mask_all,  rng_seed=RNG_SEED + 82)
...
# -----------------------------
# run Family D hard-negative univariate audit
# -----------------------------
# Run separate long-side and short-side audits under the hard-negative design.
famD_long_hard_audit = audit_one_side_hard(
    famD_long_cols,
    "y_long",
    "long",
    train_sample_long,
...
famD_short_hard_audit = audit_one_side_hard(
    famD_short_cols,
    "y_short",
    "short",
    train_sample_short,
...
# Show the highest-ranked long-side Family D features under the hard-negative design.
print("\nTop Family D long features, hard negatives")
print(
    famD_long_hard_audit[
        [
            "feature",
...
# Show the highest-ranked short-side Family D features under the hard-negative design.
print("\nTop Family D short features, hard negatives")
print(
    famD_short_hard_audit[
        [
            "feature",
            "n_train", "pos_train", "neg_train",
            "n_test", "pos_test", "neg_test",
...
D_long_uni_hard = famD_long_hard_audit.copy()
D_short_uni_hard = famD_short_hard_audit.copy()