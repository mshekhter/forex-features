# Family B hard-negative univariate side audit.
#
# Purpose:
# Evaluate each side-native Family B feature one at a time against frozen
# episode-entry labels, but under a harder negative set than the broad audit.
#
# Label definition:
# y_long and y_short are sparse entry-bar labels only.
# They mark where retained long and short episodes begin on the bar index.
# They are not bar-by-bar trade outcome labels.
#
# Side separation:
# Long audit uses only Family B up-side features.
# Short audit uses only Family B down-side features.
# This cell does not mix feature sides.
#
# Hard-negative design:
# Negatives are not all non-entry bars.
# They are sampled from bars that are outside a blocked buffer around any
# retained entry, then matched by calendar year to the positive sample.
# The target negative-to-positive ratio is fixed at 10:1 within each year.
#
# Why the buffer exists:
# Bars near retained entries are excluded from the negative pool so the model
# is tested against cleaner non-entry contexts, not bars sitting immediately
# beside true episode starts.
#
# Train/test protocol:
...
#
# Output meaning:
# Each output row is one univariate feature audit under the hard-negative
# sample construction.
...
#
# Saved tables:
# B_long_uni_hard and B_short_uni_hard keep the full long-side and short-side
...
# Prefer the reduced feature frame when it exists.
# Otherwise use the full feature frame.
if "df_feat_reduced" in globals():
    df_feat_B_src = df_feat_reduced.copy()
elif "df_feat" in globals():
    df_feat_B_src = df_feat.copy()
else:
...
# -----------------------------
# exact Family B side-native columns
# -----------------------------
# Long audit uses only up-native Family B columns.
famB_long_cols = sorted([
    c for c in df_audit.columns
    if c.startswith("famB_") and ("_up_" in c or c.endswith("_up"))
])

# Short audit uses only down-native Family B columns.
famB_short_cols = sorted([
    c for c in df_audit.columns
    if c.startswith("famB_") and ("_dn_" in c or c.endswith("_dn"))
])

print("Family B long-native cols :", len(famB_long_cols))
print("Family B short-native cols:", len(famB_short_cols))
...
# Mark every retained entry bar, regardless of side.
...
train_sample_long, train_pos_long, train_neg_long = build_hard_sample_mask("y_long", train_mask_all, rng_seed=RNG_SEED + 31)
test_sample_long,  test_pos_long,  test_neg_long  = build_hard_sample_mask("y_long", test_mask_all,  rng_seed=RNG_SEED + 32)

train_sample_short, train_pos_short, train_neg_short = build_hard_sample_mask("y_short", train_mask_all, rng_seed=RNG_SEED + 41)
test_sample_short,  test_pos_short,  test_neg_short  = build_hard_sample_mask("y_short", test_mask_all,  rng_seed=RNG_SEED + 42)
...
# -----------------------------
# run Family B hard-negative univariate audit
# -----------------------------
# Run separate long-side and short-side audits under the hard-negative design.
famB_long_hard_audit = audit_one_side_hard(
    famB_long_cols,
    "y_long",
    "long",
    train_sample_long,
...
famB_short_hard_audit = audit_one_side_hard(
    famB_short_cols,
    "y_short",
    "short",
    train_sample_short,
...
# Show the highest-ranked long-side features under the hard-negative design.
print("\nTop Family B long features, hard negatives")
print(
    famB_long_hard_audit[
        [
            "feature",
...
# Show the highest-ranked short-side features under the hard-negative design.
print("\nTop Family B short features, hard negatives")
print(
    famB_short_hard_audit[
        [
            "feature",
            "n_train", "pos_train", "neg_train",
            "n_test", "pos_test", "neg_test",
...
B_long_uni_hard = famB_long_hard_audit.copy()
B_short_uni_hard = famB_short_hard_audit.copy()