# Family D univariate side audit.
#
# Purpose:
...
#
# Output meaning:
# Each output row is one univariate Family D feature audit.
# Ranking is driven first by out-of-sample test AUC,
# then by out-of-sample top-decile lift,
# then by oriented train AUC.
#
# Saved tables:
# D_long_uni and D_short_uni keep the full long-side and short-side audit
# results for downstream review, selection, and later modeling.

import numpy as np
import pandas as pd

# Require the master 5 minute bar frame used for timestamp-to-row alignment.
# Require the retained episode table that provides entry timestamps and sides.
...
# Run separate long-side and short-side Family D univariate audits.
famD_long_audit = audit_one_side(famD_long_cols, "y_long", "long")
famD_short_audit = audit_one_side(famD_short_cols, "y_short", "short")

# Show the highest-ranked long-side Family D features.
print("\nTop Family D long features")
print(
    famD_long_audit[
        [
            "feature",
...
# Show the highest-ranked short-side Family D features.
print("\nTop Family D short features")
print(
    famD_short_audit[
        [
            "feature",
...
D_long_uni = famD_long_audit.copy()
D_short_uni = famD_short_audit.copy()