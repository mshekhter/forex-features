# Family B univariate side audit.
#
# Purpose:
...
# They are not bar-by-bar outcome labels.
#
# Side separation:
...
#
# Output meaning:
# Each output row is one univariate feature audit.
# Ranking is driven first by out-of-sample test AUC,
# then by out-of-sample top-decile lift,
# then by oriented train AUC.
#
# Saved tables:
# B_long_uni and B_short_uni keep the full long-side and short-side audit
...
# Prefer the reduced feature frame when present.
...
# -----------------------------
# frozen side labels aligned to df5 / feature frame index
# -----------------------------
# Convert df5 timestamps to a lookup index, then map each retained episode
# entry timestamp back to its exact row in df5.
...
# Build sparse side-specific entry labels on the df5 index.
...
# time split
# -----------------------------
# Time-based split used throughout the audit.
# This split is global and does not depend on feature availability.
...
# scoring helpers
# -----------------------------
# Rank-based AUC for a binary label and a numeric score.
...
# -----------------------------
# run Family B broad-negative univariate audit
# -----------------------------
# Run separate long-side and short-side univariate audits.
famB_long_audit = audit_one_side(famB_long_cols, "y_long", "long")
famB_short_audit = audit_one_side(famB_short_cols, "y_short", "short")

# Show the highest-ranked long-side features under the test-first ranking.
print("\nTop Family B long features")
print(
    famB_long_audit[
        [
            "feature",
...
# Show the highest-ranked short-side features under the test-first ranking.
print("\nTop Family B short features")
print(
    famB_short_audit[
        [
            "feature",
...
B_long_uni = famB_long_audit.copy()
B_short_uni = famB_short_audit.copy()